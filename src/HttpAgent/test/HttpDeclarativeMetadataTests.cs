...
public class HttpDeclarativeMetadataTests
{
    [Fact]
    public void New_Invalid_Parameters()
    {
        Assert.Throws<ArgumentNullException>(() => new HttpDeclarativeMetadata(null!, null!));
        var method = typeof(IHttpDeclarativeTest).GetMethod(nameof(IHttpDeclarativeTest.Method1));
        Assert.Throws<ArgumentNullException>(() => new HttpDeclarativeMetadata(method!, null!));
    }
    ...
    {
        var method = typeof(IHttpDeclarativeTest).GetMethod(nameof(IHttpDeclarativeTest.Method1))!;
        var methodMetadata = new HttpDeclarativeMetadata(method, method.DeclaringType!);
        ...
        var methodMetadata2 = new HttpDeclarativeMetadata(method2!, typeof(IChildService));
        Assert.Single(methodMetadata2.InterfaceAttributes?.OfType<HttpClientNameAttribute>() ?? []);

        var methodMetadata3 = new HttpDeclarativeMetadata(method2!, typeof(IChildService2));
