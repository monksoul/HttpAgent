...
public class HttpCurlParsingContextTests
{
    [Fact]
    public void New_Invalid_Parameters() =>
        Assert.Throws<ArgumentNullException>(() => new HttpCurlParsingContext(null!));

    [Fact]
    public void New_ReturnOK()
    {
        var tokens = new List<string> { "curl", "-X", "POST" };
        var context = new HttpCurlParsingContext(tokens);
    ...
    public void HasNext_ReturnOK()
    {
        ...
        context = new HttpCurlParsingContext(tokens);
        Assert.True(context.HasNext);
        context.Advance();
        Assert.False(context.HasNext);
    }
