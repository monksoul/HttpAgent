...
[**查看高清架构图**](https://gitee.com/dotnetchina/HttpAgent/blob/master/drawio/HttpAgent.drawio)
...
- **请求弹性与容错策略**：内置请求重试机制，支持状态码/异常触发、自定义间隔与指数退避；支持单请求超时控制与回调；可扩展断路器、熔断、后备（Fallback）等高级弹性模式。
- **`Access Token` 自动管理**：内置 `Access Token` 自动刷新与注入机制，支持自定义 `Token` 提供器和配置器，请求时自动携带
  `Token`，并在 `401` 响应时自动重试。
- **接口调用配额限制**：支持按天、按周、按月、生命周期总次数等多种窗口策略，可自定义配额策略；超限时立即中断请求，防止第三方
  API 滥用。
- **`ETag` 响应缓存**：自动缓存带 `ETag` 的响应，后续请求在服务器返回 `304 Not Modified` 时直接复用缓存，减少数据传输。
...
- **`cURL` 命令解析**：支持从原生 `cURL` 命令字符串一键发起 HTTP 请求，覆盖常用选项并可自由扩展自定义标志。
- **声明式请求**：通过简单的属性注解或接口定义来指定请求的行为。
- **响应转换灵活性**：提供常用的 `HTTP` 响应转换器，并支持自定义转换逻辑。
- **高性能与资源管理**：采用内存优化技术和连接池管理，确保低资源消耗与高性能表现。
- **响应自动解压**：内置支持 `gzip`、`deflate`、`brotli` 和 `zstd` 响应内容自动解压，减少手动处理负担并节省带宽。
- **架构设计**：架构设计灵活，易于使用与扩展。
- **跨平台无依赖**：支持跨平台运行，无需外部依赖。
- **应用类型支持**：支持任何应用程序类型，包括但不限于控制台、Web、WebAssembly、WinForms、WPF、MAUI。
